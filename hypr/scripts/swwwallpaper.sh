...
wallSet="${XDG_CONFIG_HOME:-$HOME/Hyprdots/dotfiles}/swww/wall.set"
wallBlr="${XDG_CONFIG_HOME:-$HOME/Hyprdots/dotfiles}/swww/wall.blur"
wallRfi="${XDG_CONFIG_HOME:-$HOME/Hyprdots/dotfiles}/swww/wall.rofi"
...
if [ ! -f "$fullPath" ] ; then
    if [ -d "${XDG_CONFIG_HOME:-$HOME/Hyprdots/dotfiles}/swww/$curTheme" ] ; then
        wallPath="${XDG_CONFIG_HOME:-$HOME/Hyprdots/dotfiles}/swww/$curTheme"
        mapfile -d '' Wallist < <(find ${wallPath} -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" \) -print0 | sort -z)
        fullPath="${Wallist[0]}"
    else
