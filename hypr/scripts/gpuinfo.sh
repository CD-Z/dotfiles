...
amd_GPU() { #? Funtion to query amd GPU
  ...
  amd_output=$(python3 ~/Hyprdots/dotfiles/hypr/scripts/amdgpu.py)
if [[ ! ${amd_output} == *"No AMD GPUs detected."* ]] && [[ ! ${amd_output} == *"Unknown query failure"* ]]; then #! This will be changed if "(python3 ~/Hyprdots/dotfiles/hypr/scripts/amdgpu.py)" Changes!
...
# elif  #? Can add another Layer of query if "~/Hyprdots/dotfiles/hypr/scripts/amdgpu.py" fails
